 ...
 *
 * 此测试套件专注于SpinalHDL随机数生成机制的隔离性和一致性验证，
 * 是对SpinalSimRandomizeTester.scala中基本功能测试的重要补充。
 *
 * Key differences from SpinalSimRandomizeTester:
 * 与SpinalSimRandomizeTester的主要区别：
 * - SpinalSimRandomizeTester: Tests basic randomize() functionality
 * - This suite: Tests isolation, consistency, and multi-threading behavior
 *
 * Test Coverage:
 * 测试覆盖范围：
 * 1. SpinalHDL's simRandom isolation between simulation runs
 *    SpinalHDL的simRandom在仿真运行间的隔离
